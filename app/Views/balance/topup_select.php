<?php $title=__('topup_balance').' - '.__('papam_voip_panel'); require dirname(__DIR__).'/partials/header.php'; ?>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="text-center mb-8 animate-fade-in">
        ...
        <p class="text-slate-600 dark:text-slate-400"><?= __('select_group_for_topup') ?></p>
      </div>
      ...
      <div class="flex items-center justify-center mb-8">
        <div class="flex items-center space-x-4">
          <div class="flex items-center">
            ...
            <span class="ml-2 text-sm font-medium text-indigo-600"><?= __('select_group') ?></span>
          </div>
          ...
          <div class="flex items-center">
            ...
            <span class="ml-2 text-sm font-medium text-slate-500 dark:text-slate-400"><?= __('determine_amount') ?></span>
          </div>
        </div>
      </div>
        ...
        <div class="bg-white/80 dark:bg-slate-800/80 backdrop-blur-lg rounded-2xl shadow-xl border border-slate-200/50 dark:border-slate-700/50 overflow-hidden">
          <div class="p-8">
            <div class="text-center mb-6">
              ...
              <h2 class="text-2xl font-semibold text-slate-800 dark:text-slate-200 mb-2"><?= __('group_selection') ?></h2>
              <p class="text-slate-600 dark:text-slate-400"><?= __('select_group_to_topup') ?></p>
            </div>
            ...
            <form method="get" action="<?= \App\Helpers\Url::to('/groups/topup') ?>" class="space-y-3 bg-white dark:bg-slate-800 p-4 rounded-xl shadow">
              ...
              <button class="w-full bg-gradient-to-r from-indigo-600 to-blue-600 text-white rounded p-2"><?= __('continue') ?></button>
            </form>
          </div>
        </div>
        ...
        <div class="grid md:grid-cols-2 gap-4 mt-6">
          <div class="bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm rounded-xl p-4 border border-slate-200/30 dark:border-slate-700/30">
            <div class="flex items-center gap-3 mb-2">
              ...
              <h3 class="font-semibold text-slate-800 dark:text-slate-200"><?= __('secure_transaction') ?></h3>
            </div>
            <p class="text-sm text-slate-600 dark:text-slate-400"><?= __('all_topup_operations_secure') ?></p>
          </div>
          ...
          <div class="bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm rounded-xl p-4 border border-slate-200/30 dark:border-slate-700/30">
            <div class="flex items-center gap-3 mb-2">
              ...
              <h3 class="font-semibold text-slate-800 dark:text-slate-200"><?= __('instant_effective') ?></h3>
            </div>
            <p class="text-sm text-slate-600 dark:text-slate-400"><?= __('balance_reflects_immediately') ?></p>
          </div>
        </div>
