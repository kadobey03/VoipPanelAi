  ...
  <div class="container mx-auto p-4 max-w-lg">
    ...
    <form method="post" class="space-y-3 bg-white dark:bg-gray-800 p-4 rounded shadow">
      <div>
        ...
        <input name="name" required class="w-full border rounded p-2 bg-white dark:bg-gray-800" value="<?= htmlspecialchars($group['name']) ?>">
      </div>
      <div>
        <label class="block text-sm mb-1">Margin %</label>
        <input type="number" step="0.01" name="margin" class="w-full border rounded p-2 bg-white dark:bg-gray-800" value="<?= htmlspecialchars((string)$group['margin']) ?>">
      </div>
      <div class="text-sm text-gray-500">Çağrı maliyetine margin uygulanır: fiyat = cost_api * (1 + margin/100)</div>
