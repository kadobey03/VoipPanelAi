<?php $title=__('balance_topup_requests').' - '.__('papam_voip_panel'); require dirname(__DIR__).'/partials/header.php'; ?>
  <?php $isSuper = isset($_SESSION['user']) && ($_SESSION['user']['role']??'')==='superadmin'; ?>
  <div class="flex items-center justify-between mb-4">
    <h1 class="text-2xl font-bold flex items-center gap-2"><i class="fa-solid fa-circle-plus text-emerald-600"></i> <?= __('balance_topup_requests') ?></h1>
  </div>
    ...
    <table class="min-w-full text-sm">
      <thead class="bg-slate-50 dark:bg-slate-900/40">
        <tr class="border-b border-slate-200 dark:border-slate-700 text-left">
          ...
          <th class="p-2"><?= __('method') ?></th>
          <th class="p-2"><?= __('status') ?></th>
          <th class="p-2"><?= __('note') ?></th>
          <th class="p-2"><?= __('receipt') ?></th>
          <th class="p-2"><?= __('date') ?></th>
          <?php if ($isSuper): ?><th class="p-2"><?= __('actions') ?></th><?php endif; ?>
        </tr>
      </thead>
      <tbody>
        ...
        <tr class="border-b border-slate-100 dark:border-slate-700/60">
          ...
          <td class="p-2 capitalize">
            ...
            <?php if ($isCrypto && $it['status'] === 'pending' && !$isExpired && !empty($it['crypto_payment_id'])): ?>
              <br><a class="text-blue-600 hover:underline text-xs mt-1 inline-block" href="<?= \App\Helpers\Url::to('/topups/crypto/continue') ?>?payment_id=<?= (int)$it['crypto_payment_id'] ?>"><?= __('view') ?></a>
            <?php endif; ?>
          </td>
          ...
          <td class="p-2">
            <?php if (!empty($it['receipt_path'])): ?>
              <a class="text-blue-600 hover:underline" href="<?= \App\Helpers\Url::to('/topups/receipt') ?>?id=<?= (int)$it['id'] ?>" target="_blank"><?= __('view') ?></a>
            <?php endif; ?>
          </td>
          <?php if ($isSuper): ?>
          <td class="p-2 space-x-2">
            ...
            <form method="post" action="<?= \App\Helpers\Url::to('/topups/approve') ?>" style="display:inline">
              ...
              <button class="px-2 py-1 rounded bg-emerald-600 text-white"><?= __('approve') ?></button>
            </form>
            <form method="post" action="<?= \App\Helpers\Url::to('/topups/reject') ?>" style="display:inline">
              <input type="hidden" name="id" value="<?= (int)$it['id'] ?>">
              <button class="px-2 py-1 rounded bg-rose-600 text-white"><?= __('reject') ?></button>
            </form>
            <?php endif; ?>
          </td>
          <?php endif; ?>
