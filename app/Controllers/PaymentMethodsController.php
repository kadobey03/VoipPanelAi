...
class PaymentMethodsController {
    private function start(){ if (session_status()===PHP_SESSION_NONE) session_start(); if (!isset($_SESSION['user'])) { \App\Helpers\Url::redirect('/login'); } if (($_SESSION['user']['role']??'')!=='superadmin') { http_response_code(403); echo Lang::get('unauthorized'); exit; } }
    ...
    public function create(){
        ...
        if ($_SERVER['REQUEST_METHOD']==='POST'){
            ...
            if($name===''){ $error=Lang::get('name_required'); } else { $stmt=$db->prepare('INSERT INTO payment_methods (name, method_type, details, fee_percent, fee_fixed, active) VALUES (?,?,?,?,?,?)'); $stmt->bind_param('sssddi', $name,$type,$details,$fee_percent,$fee_fixed,$active); $ok=$stmt->execute()?Lang::get('saved'):Lang::get('save_failed'); $stmt->close(); }
        }
    ...
    public function edit(){
        ...
        if ($_SERVER['REQUEST_METHOD']==='POST'){
            $name=trim($_POST['name']??''); $type=trim($_POST['method_type']??'manual'); $details=trim($_POST['details']??''); $fee_percent=(float)($_POST['fee_percent']??0); $fee_fixed=(float)($_POST['fee_fixed']??0); $active=isset($_POST['active'])?1:0;
            if($name===''){ $error=Lang::get('name_required'); } else { $stmt=$db->prepare('UPDATE payment_methods SET name=?, method_type=?, details=?, fee_percent=?, fee_fixed=?, active=? WHERE id=?'); $stmt->bind_param('sssddii', $name,$type,$details,$fee_percent,$fee_fixed,$active,$id); $ok=$stmt->execute()?Lang::get('updated'):Lang::get('update_failed_short'); $stmt->close(); }
        }
