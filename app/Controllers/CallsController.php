...
class CallsController {
    ...
    public function syncCron(){
        ...
        try {
            ...
            $page = 1;
            while (true) {
                // Apply API time offset if configured
                $offsetMin = (int)(getenv('API_TIME_OFFSET_MINUTES') ?: 0);
                $apiFrom = \App\Helpers\Time::applyOffset($from, $offsetMin);
                $apiTo   = \App\Helpers\Time::applyOffset($to, $offsetMin);
                $cdrs = $api->getCallHistoryFilter($apiFrom, $apiTo, null, null, $page);
                $pages++;
                if ($debug) { $trace[] = ['request'=>['from'=>$from,'to'=>$to,'api_from'=>$apiFrom,'api_to'=>$apiTo,'page'=>$page], 'response'=>$cdrs]; }
